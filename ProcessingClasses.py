...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        try:
            ...
            print("Needs an Integer as ID")
        raise ('The '+ str(cd_idx) + ' CD/Album does not exist')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        # Add code as required
        trackPosition = track_info[0]
        trackTitle = track_info[1]
        trackLength = track_info[2]
        try:
            trackPosition = int(trackPosition)
        except:
            raise Exception('Position must be an Integer')
        track = DC.Track(trackPosition, trackTitle, trackLength)
        cd.add_track(track)   
